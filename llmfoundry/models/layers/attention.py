...
@attention_classes.register_class("selective_multihead_attention")
class SelectiveMultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.d_model = d_model
        self.n_heads = n_heads
        self.kv_n_heads = n_heads  # Selective MHA uses n_heads for kv_n_heads
        self.sliding_window_size = sliding_window_size
        self.reuse_kv_layer_idx = reuse_kv_layer_idx
        self.attn_logit_softcapping = attn_logit_softcapping
    ...
    def forward(
        ...
        context, attn_weights, past_key_value = self.attn_fn(
            ...
            key,
            value,
            n_heads=len(self.trainable_heads),
            kv_n_heads=len(self.trainable_heads),
            past_key_value=past_key_value,
            softmax_scale=self.softmax_scale,
            attn_bias=attn_bias,
            is_causal=is_causal,
        ...
        output_t = context @ trainable_o.t()

        if self.out_proj.bias is not None:
            output_t += self.out_proj.bias

        return output_t, attn_weights, past_key_value
    ...
    def get_qkv(
        ...
        if self.fused_qkv:
            ...
            # This is the place to separate the computation.
            ...
            trainable_w = torch.cat(
                [
                    self.Wqkv.weight[
                        i * self.d_model
                        + j * self.head_dim : i * self.d_model
                        + (j + 1) * self.head_dim,
                        :,
                    ]
                    for j in self.trainable_heads
